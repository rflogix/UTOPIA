...
CKEDITOR.dialog.add( 'image2', function( editor ) {
	...
	var regexGetSizeOrEmpty = /(^\s*(\d+)(px)?\s*$)|^$/i,
		...
		blnFileUploadYN, // ◀◀◀◀◀ 2017.01.18 박찬원 추가
	...
	function onChangeSrc(p_this) {
		var value = p_this.getValue();
	...
	return {
		...
		onShow: function() {
			...
			// ▼▼▼▼▼ 2017.01.18 박찬원 추가 ▼▼▼▼▼
			blnFileUploadYN = false;
			$("div.cke_dialog_ui_input_file").parent().parent().css("display","none");
			$("a.cke_dialog_ui_fileButton").parent().css("display","none");
			$("#divImageShow_cke").parent().css("width","100%").css("display","none");
			if (widget.data.src) {
				if (widget.data.src != "") {
					$("#divImageShow_cke").css("background-image","url('"+widget.data.src+"')").show();
					$("#divImageShow_cke").parent().css("display","table-cell");
				}
			}
			// ▲▲▲▲▲ 2017.01.18 박찬원 추가 ▲▲▲▲▲
		},
			...
			{id:"info"
				...
				,elements:[
					{type: "text"
						...
						// ▼▼▼▼▼ 2017.01.18 박찬원 수정 ▼▼▼▼▼
						,label: "이미지 URL"
						,onKeyup: function() {onChangeSrc(this);}
						,onChange: function() {
							if (this.getValue() != "") {
								var element;
								element = this.getDialog().getContentElement("info","src").getElement().getChild(1).getChild(0).getChild(0);
								element.removeAttributes("disabled");
								element = this.getDialog().getContentElement("info","btnFileUpload").getElement().getChild(0);
								element.setHtml("파일 업로드");
								
								$("#divImageShow_cke").css("background-image","url('"+this.getValue()+"')").show();
								$("#divImageShow_cke").parent().css("display","table-cell");
						...
						,commit: function( widget ) {
							widget.setData( 'class', "cke_upload_img" ); // 새로 추가
					...
					// ▼▼▼▼▼ 2017.01.18 박찬원 추가 ▼▼▼▼▼
					,{type: "button" // 자동으로 생성되는 iframe 안에 file 클릭되도록 사용자 정의 버튼 생성
						,id:"btnFileUpload"
						,label: "파일 업로드"
						,style:"display:inline-block; width:calc(100% - 2px);"
						,onClick: function() {
							if (blnFileUploadYN == false) {
								$("iframe.cke_dialog_ui_input_file").contents().find("input[type='file']")[0].click(); // file 클릭 (id:upload)
							} else {
								alert("파일 업로드중 입니다");
							}
						}
					}
					,{type: "html" // 업로드된 이미지 바로 볼 수 있도록 div 추가
						,html:"<div id='divImageShow_cke' style='width:100%; background-size:contain; background-position:center center; background-repeat:no-repeat;'><img src='image/common/bg_1x1.png' style='width:100%;' /></div>"
					}
					// ▲▲▲▲▲ 2017.01.18 박찬원 추가 ▲▲▲▲▲
					,{type:"file"
						,id:"upload"
						,label:lang.btnUpload
						,onChange:function(e) { // ◀◀◀◀◀ 2017.01.18 박찬원 추가 : file 에서 파일 선택되었다면 fileButton 버튼 클릭
							if (blnFileUploadYN == false) {
								blnFileUploadYN = true;
								
								var element;
								element = this.getDialog().getContentElement("info","src").getElement().getChild(1).getChild(0).getChild(0);
								element.setAttribute("disabled","disabled");
								element = this.getDialog().getContentElement("info","btnFileUpload").getElement().getChild(0);
								element.setHtml("파일 업로드중 입니다...");
								
								$("a.cke_dialog_ui_fileButton")[0].click(); // fileButton 클릭
						...
						,children: [
							{
								...
								commit: function( widget ) {
									//widget.setData( 'width', this.getValue() ); // ◀◀◀◀◀ 2017.01.18 박찬원 수정
								}
							}
							,{
								type: 'text',
								...
								commit: function( widget ) {
									//widget.setData( 'height', this.getValue() ); // ◀◀◀◀◀ 2017.01.18 박찬원 수정
								}
							},
							{
								id: 'lock',
